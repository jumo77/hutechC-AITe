import {answer, difficulty} from "../../../../../../../component/const/ForShort";
import {Context} from "../Context";
import {useContext} from "react";
import {Activate, SavedShortId} from "../../../../../../../component/const/ForCreate";

export default function SaveButton(){

    const {problems, metaData, setMetaData, hideShort, setClicked} = useContext(Context)

    const onClick=()=>{
        setMetaData(prev=>({
            ...prev,
            max: problems.reduce((accumulator, currentObject) => {
        ...
        problems.forEach((it, index)=>{
            ...
            if (it.answer!==undefined && it.answer === answer){
                i = index
                valid=false
                problem= "정답"
                return
            }
            if (it.answers!==undefined && it.answers.includes("")){
                i = index
                valid=false
                problem= "정답"
            }
        })
        if (problems.length === 0) alert("문제가 출제되지 않았습니다.")
        else if(metaData.pass > metaData.max) alert("만점보다 합격점이 더 높습니다.")
        else if(metaData.pass===0) alert("합격점을 입력해주세요.")
        else if (!valid) alert((i + 1) + '번 문제의 '+problem+'값이 설정되지 않았습니다. 확인해주세요.')
        else {
            setClicked(prev=>prev++)
            hideShort()
        }
        Activate(SavedShortId)
    }
