...
export default function SubmitTrans() {
    const {
        metaData,
        file,
        setFile,
    ...
    useEffect(() => {
        if (file) switch (file?.name.slice(".")[1]) {
            case "pdf":
                loadPdf()
                    .then(() => {
    ...
    useEffect(() => {
        if (original !== "" && translate) {Translator.forEach(it => {
    ...
    return (<>
        <section>
            <table>
                <thead>
                <tr>
                    <th><select onChange={e => setFileType(e.target.value)}>
                        {UploadFileType.map((it, index) =>
                            (<option key={it + index.toString()} selected={index === 0} disabled={index === 0}
                                     value="it">{it}</option>))}
                    </select></th>
                </tr>
                </thead>
                ...
                <tr>
                    <td id="file"
                        ...
                        <input disabled={fileType === UploadFileType[0]} {...getInputProps()}/>
                    ...
                    {!file ? <>
                        ...
                        : <><td>
                            {original === "" ?
                                ...
                                <textarea value={original} style={{width: "100%"}}
                                          onChange={event =>
                                              setOriginal(event.target.value)}/>}
                        </td><td>
                            {answer === "" ?
                                <img src="https://media.tenor.com/On7kvXhzml4AAAAj/loading-gif.gif" alt=""/> :
                                <textarea value={answer} style={{width: "100%"}}
                                          onChange={event =>
