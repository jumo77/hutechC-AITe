...
export default function TestShort(){
    ...
    return (
        <>
            <table>
                <thead>
                <tr>{fixed.header?.map((it, index)=><th key={it+index.toString()} dangerouslySetInnerHTML={{__html: it}}/> )}</tr>
                </thead>
                <tbody>
                <tr>{fixed.metaData?.map((it,index)=><td key={it+index.toString()} dangerouslySetInnerHTML={{__html:it}}/>)}</tr>
                </tbody>
            </table>
            <TopBar title="출제 내용"/>
